...
public:
    ...
    {
        if (position < 0 || position >= size) throw out_of_range("¬ыход за пределы индекса");
    ...
    void pop_front()
    {
        if (isEmpty()) throw std::exception("”даление неопределенного указател€");
    ...
    {
        if (position < 0 || position >= size - 1) throw out_of_range("¬ыход за пределы индекса");
    ...
    bool operator==(const List& list) const  // дл€ теста
    {
    ...
    bool operator!=(const List& list) const  // дл€ тестов
    {
        return !(*this == list);
    }
    ...
    T& at(int position) // контролируемый доступ
    {
        if (position < 0 || position >= size) throw out_of_range("¬ыход за пределы индекса");
    ...
    const T& at(int position) const // контролируемый доступ
    {
        if (position < 0 || position >= size) throw out_of_range("¬ыход за пределы индекса");
